...
export default function interfaceTabsNav() {
  const elements = Array.from(
    document.querySelectorAll<HTMLElement>(".interface__tabs-nav")
  );
    ...
    let instance = new Swiper(container, {
      ...
      navigation: {
        prevEl: element.querySelector<HTMLButtonElement>(
          ".interface__tabs-nav-arrow--prev"
        ),
        nextEl: element.querySelector<HTMLButtonElement>(
          ".interface__tabs-nav-arrow--next"
        ),
      },
      on: {
        init(swiper) {
